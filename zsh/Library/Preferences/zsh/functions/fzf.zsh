# Open file
# fe [FUZZY PATTERN] - Open the selected file with the visual studio code editor
#   - Bypass fuzzy finder if there's only one match (--select-1)
#   - Exit if there's no match (--exit-0)
function fe {
  local files
  IFS=$'\n' files=($(fzf --query="$1" --multi --select-1 --exit-0 --preview 'bat --style=numbers --color=always {}')) &&
...
function cds {
  local file
  local dir
  file=$(fzf --query "$1" --no-multi --layout=reverse --height=40% --preview 'bat --style=numbers --color=always {}') && dir=$(dirname "$file") && cd "$dir"
}
